...
import { SoccerLevel, SoccerTeam } from '../data/gameData';
...
interface SoccerGameScreenProps {
  ...
  onTeamSolved: (levelId: string, teamId: string) => void;
}

const SoccerGameScreen: React.FC<SoccerGameScreenProps> = ({
  level,
  coins,
  onBack,
  onCoinsChange,
  ...
  // Get unsolved teams for selection
  const unsolvedTeams = level.teams.filter(team => !team.solved);
  const solvedCount = level.teams.filter(team => team.solved).length;

  useEffect(() => {
    if (selectedTeam) {
      setupNewTeam(selectedTeam);
    }
  }, [selectedTeam]);

  const setupNewTeam = (team: SoccerTeam) => {
  ...
  const selectTeam = (team: SoccerTeam) => {
  ...
  const checkAnswer = () => {
    ...
    if (answer === selectedTeam.name) {
      // Correct answer!
      const coinsEarned = 10 + (selectedTeam.difficulty * 5);
      onCoinsChange(coins + coinsEarned);
      onTeamSolved(level.id, selectedTeam.id);
  ...
  const useHint = (type: 'reveal' | 'remove' | 'answer') => {
    ...
    if (type === 'reveal') {
      ...
      const correctName = selectedTeam.name;
      ...
      const correctLetters = selectedTeam.name.split('');
      ...
      // Show full answer
      setCurrentAnswer(selectedTeam.name.split(''));
      setAvailableLetters([]);
    }
  };
      ...
      <div className="min-h-screen bg-gradient-to-br from-green-900 via-blue-900 to-black text-white">
        <div className="container mx-auto px-4 py-6">
          ...
          <div className="text-center mb-8">
            ...
            {level.type === 'themed' && (
              <p className="text-green-300 text-lg">Theme: {level.theme}</p>
            )}
            <p className="text-gray-300 mt-2">
              Progress: {solvedCount}/{level.teams.length} completed
            </p>
          </div>
              ...
              <div
                ...
                className={`relative bg-gray-800 rounded-lg p-4 cursor-pointer transition-all hover:scale-105 ${
                  team.solved 
                    ...
                    : 'hover:bg-gray-700 border-2 border-transparent hover:border-green-500'
                }`}
              >
                {/* Team Logo */}
                <div className="aspect-square mb-3 rounded-lg overflow-hidden bg-white p-2">
                  <img
                    src={team.logoUrl}
                    alt="Mystery Team"
                    className={`w-full h-full object-contain ${
                      team.solved ? 'filter-none' : 'filter blur-sm'
                    }`}
                  />
                </div>
                ...
                <div className="text-center">
                  {team.solved ? (
                    <p className="text-green-300 font-bold text-sm">{team.name}</p>
                  ) : (
                    <p className="text-gray-300 text-sm">Mystery Team</p>
                  )}
                  
                  <p className="text-xs text-gray-400 mt-1">{team.league}</p>
                  
                  <div className="flex justify-center mt-2">
                    {[...Array(team.difficulty)].map((_, i) => (
          ...
          <div className="text-center text-gray-300">
            <p>Tap any unsolved team to start guessing!</p>
            <p className="text-sm mt-2">Use the logo as a hint to spell out the team's name</p>
          </div>
        </div>
      </div>
    );
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-green-900 via-blue-900 to-black text-white">
      <div className="container mx-auto px-4 py-6">
        ...
        {/* Current Team */}
        <div className="text-center mb-8">
          <div className="w-48 h-48 mx-auto mb-4 rounded-lg overflow-hidden bg-white p-4">
            <img
          ...
          <div className="flex justify-center mb-2">
            {[...Array(selectedTeam.difficulty)].map((_, i) => (
              <Star key={i} size={20} className="text-yellow-500 fill-current" />
            ))}
          </div>
          
          <p className="text-gray-300">League: {selectedTeam.league}</p>
          <p className="text-gray-400 text-sm">Country: {selectedTeam.country}</p>
        </div>

        {/* Answer Slots */}
        <div className="mb-8">
          <h3 className="text-xl font-bold text-center mb-4">Spell the team name:</h3>
